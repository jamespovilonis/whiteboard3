...
class AuditServiceTests(unittest.TestCase):
    def test_agreement_writes_artifacts_and_empty_discrepancies(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn("queueTelemetry", metadata)
            self.assertIsInstance(metadata["queueTelemetry"]["queueDepthAtStart"], int)
            self.assertIsInstance(metadata["queueTelemetry"]["queueMs"], (int, float))
            self.assertIn("vlmRequestLifecycle", metadata)
            self.assertEqual(metadata["vlmRequestLifecycle"]["attemptCount"], 1)
            self.assertEqual(metadata["vlmRequestLifecycle"]["attempts"][0]["status"], "complete")
            self.assertIn("requestStartedAt", metadata["vlmRequestLifecycle"]["attempts"][0])
            ...
            self.assertIn("queueTelemetry", summary)
            self.assertIsInstance(summary["queueTelemetry"]["queueDepthAtStart"], int)
            self.assertIn("vlmRequestLifecycle", summary)
            self.assertEqual(summary["vlmRequestLifecycle"]["attemptCount"], 1)
